...
try:
    ...
    print(f"WARN: Could not import browser_use.Agent or patch dependencies: {e}. Internal patch may not apply correctly.")
...
if AGENT_CLASS_AVAILABLE:
    ...
    if hasattr(Agent, target_method_name):
        ...
        async def patched_run_planner(self, *args, **kwargs):
            ...
            print(f"DEBUG Patch: Intercepting {target_method_name}. FORCING MAIN LLM FOR PLANNING.")
        ...
        print(f"WARN: Could not find method '{target_method_name}' on Agent class to patch. Internal patch not applied.")
else:
...
# --- Restore Patches ---
# Restore Agent Patch
if AGENT_CLASS_AVAILABLE and original_agent_run_planner and hasattr(Agent, target_method_name):
    try:
