...
def test_diagnostics_endpoint():
    ...
    try:
        ...
        response = requests.get(
            DIAGNOSTICS_URL, headers=HEADERS, timeout=60
        )  # Add timeout
        ...
        if response.status_code == 200:
            ...
            print(
                f"Diagnostics call successful, API reported unhealthy (Status Code: {response.status_code})"
            )
        else:
...
if initial_status == "pending":
    ...
    while polling_attempts < max_polling_attempts:
        ...
        try:
            ...
            print(
                f"  Attempt {polling_attempts}: An unexpected error during polling: {e}"
            )
            final_status = "error"
...
if final_status == "completed":
    ...
    try:
        ...
        final_result_data = (
            response.json()
        )  # Get the final state including the error field
        ...
        polling_error = (
            polling_error or "(Could not fetch error details for failed task)"
        )
