...
file_handler = RotatingFileHandler(
    "api.log", maxBytes=1024 * 1024 * 5, backupCount=2
)  # 5MB max size, 2 backups
...
if os.environ.get("REPL_ID"):
    app.logger.debug(
        "Replit environment detected, configuring resource limits and Playwright..."
    )
...
try:
    ...
    os.environ["BROWSER_USE_BROWSER_ARGS"] = (
        "--no-sandbox,--disable-setuid-sandbox,--disable-dev-shm-usage,--disable-gpu,--disable-software-rasterizer,--disable-extensions,--single-process,--no-zygote"
    )
...
def get_model_instance(model_name):
    ...
    try:
        if model_name.startswith("gemini-"):
            ...
            if not api_key:
                app.logger.error(
                    "Google API key not configured. Please set GOOGLE_API_KEY."
                )
            ...
            app.logger.warning(
                f"Unknown model prefix for '{model_name}'. Defaulting to gpt-4o."
            )
            api_key = os.getenv("OPENAI_API_KEY")
        if not api_key or api_key == "your_openai_api_key_here":
...
async def run_browser_task(task_id, task_description, model_name):
    ...
    try:
        ...
        app.logger.debug(
            f"Processing task {task_id}: '{task_description}' using model {model_name}"
        )
        ...
        try:
            ...
            app.logger.debug(
                f"Playwright version {getattr(playwright, '__version__', 'unknown')} and browser-use version {getattr(browser_use, '__version__', 'unknown')} found."
            )
        ...
        try:
            # --- Conditionally Apply Patch based on Environment ---
            if os.environ.get('REPL_ID'):
                app.logger.debug(f"Replit environment detected. Applying Playwright patch for task {task_id}")
                chromium_path = os.environ.get("PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH")
                if not chromium_path or not os.path.exists(chromium_path):
                    app.logger.error(f"NIX Chromium executable path not valid: {chromium_path}")
                    raise RuntimeError(f"NIX Chromium executable path not valid: {chromium_path}")

                app.logger.debug(f"Using NIX Chromium at path: {chromium_path}")
                
                # Import BrowserType only if patching
                from playwright._impl._browser_type import BrowserType 
                
                is_patched = hasattr(BrowserType.launch, '__is_patched')
                
                if not is_patched:
                    original_launch = BrowserType.launch # Capture original
                    app.logger.debug("Original Playwright launch method captured.")

                    # Define our patched launch method - ACCEPT the instance
                    def patched_launch_fixed(browser_type_instance, **kwargs):
                        app.logger.debug(
                            f"Patched Playwright launch called, forcing executablePath={chromium_path} and headless=True"
                        )
                        kwargs['executablePath'] = chromium_path # Use chromium_path from outer scope
                        ...
                        if original_launch:
                            # Call original launch directly - PASS the instance back
                            return original_launch(browser_type_instance, **kwargs)
                        else:
                    ...
                    # Apply the FIXED patch and mark it
                    patched_launch_fixed.__is_patched = True # Mark our function
                    BrowserType.launch = patched_launch_fixed
                    app.logger.debug("Playwright monkey patch applied (fixed version).")
                else:
                    app.logger.debug("Playwright patch seems already applied.")
            else:
                app.logger.debug("Not in Replit environment, skipping Playwright patch.")
            # --- End Conditional Patch ---

            # Configure and run the agent (Revert to checking for browser_config param)
            has_browser_config_param = ('browser_config' in inspect.signature(BrowserAgent.__init__).parameters)
            if has_browser_config_param:
                app.logger.debug("Using BrowserConfig to initialize Agent (no path needed here).")
                # Pass empty BrowserConfig, patch will handle path/headless
                ...
                agent = BrowserAgent(
                 ...
                 # Fallback if BrowserConfig param doesn't exist in installed version
            ...
            app.logger.error(
                f"Browser task {task_id} failed: {browser_error}", exc_info=True
            )  # Log with traceback
            ...
            # Restore patch ONLY if it was applied (original_launch captured) AND in Replit
            if original_launch and os.environ.get('REPL_ID'): 
                 # Check patch marker before restoring - avoid restoring if patch failed to apply 
                 # or if another thread restored it already in a concurrent scenario.
                 # Also need BrowserType imported if we get here.
                 try:
                     from playwright._impl._browser_type import BrowserType
                     if hasattr(BrowserType.launch, '__is_patched'):
                         BrowserType.launch = original_launch
                         app.logger.debug(f"Restored original Playwright launch method for task {task_id}.")
                     else:
                         app.logger.debug("Patch already restored or wasn't applied, skipping restoration.")
                 except NameError: 
                     pass 
                 except Exception as e:
        ...
        app.logger.error(
            f"General error processing task {task_id}: {str(e)}", exc_info=True
        )
        tasks[task_id]["status"] = "failed"
...
def create_browser_task():
    try:
        ...
        app.logger.info(
            f"Received task creation request: ID {task_id}, Model {model_name}"
        )
        ...
        try:
            ...
            def run_async_in_thread():
                ...
                app.logger.debug(
                    f"New event loop created for task {task_id} in thread."
                )
                try:
                    # Run the main async task function until it completes
                    loop.run_until_complete(
                        run_browser_task(task_id, task_description, model_name)
                    )
                    app.logger.debug(
                        f"run_browser_task {task_id} completed in background thread loop."
                    )
                except Exception as thread_e:
                    # Log errors happening within the async task execution itself
                    app.logger.error(
                        f"Error executing run_browser_task {task_id} in background thread: {thread_e}",
                        exc_info=True,
                    )
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                app.logger.debug(
                    f"Task {task_id} submitted via ThreadPoolExecutor using dedicated loop."
                )
            ...
            app.logger.error(
                f"Failed to schedule task {task_id} using ThreadPoolExecutor: {e}",
                exc_info=True,
            )
            tasks[task_id]["status"] = "failed"
        ...
        app.logger.error(
            f"Error in create_browser_task endpoint processing request: {e}",
            exc_info=True,
        )
        return (
            jsonify(
                {"error": "Internal server error during task creation processing."}
            ),
            500,
        )
...
def get_browser_task(task_id):
    ...
    for key, value in task.items():
        if key == "result":
            ...
            if isinstance(value, dict) and "text" in value:
                ...
                app.logger.debug(
                    f"Converting non-serializable result for task {task_id} to string."
                )
    ...
    app.logger.debug(
        f"[DEBUG] Type of response_task['result'] before jsonify: {type(response_task.get('result'))}"
    )
    try:
        app.logger.debug(
            f"[DEBUG] Content of response_task before jsonify: {json.dumps(response_task, indent=2, default=str)}"
        )  # Try to dump with str fallback
    except Exception as dump_error:
        app.logger.debug(
            f"[DEBUG] Could not dump response_task, content: {response_task}"
        )
...
def diagnostics():
    ...
    status_code = (
        200 if diagnostic_data.get("overall_status") == "healthy" else 503
    )  # Service Unavailable
...
if __name__ == "__main__":
    ...
    app.run(
        host="0.0.0.0", port=5001, debug=os.environ.get("FLASK_ENV") == "development"
    )
