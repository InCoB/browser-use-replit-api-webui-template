...
def get_system_resources(logger):
    ...
    try:
        ...
        try:
            ...
            logger.warning("Access denied getting open file count.")
            open_files = "Access Denied"
        except Exception as e:
            logger.warning(f"Could not get open file count: {e}")
            open_files = "Error"

        try:
            thread_count = process.num_threads()
        except psutil.AccessDenied:
            logger.warning("Access denied getting thread count.")
            thread_count = "Access Denied"
        except Exception as e:
        ...
        resources = {
            ...
            "process": {
                "pid": process.pid,
                "memory_used_mb": round(process_mem.rss / (1024**2), 2),
...
def perform_detailed_diagnostics(logger: logging.Logger):
    ...
    health_data["environment_details"]["nix_chromium_path"] = (
        nix_chromium_path if nix_chromium_path else "not set"
    )
    ...
    if nix_chromium_path and os.path.exists(nix_chromium_path):
        ...
        try:
            ...
            with sync_playwright() as p:
                # Define env for the launch call
                ...
                logger.debug(
                    f"Launching Chromium directly with executable_path: {nix_chromium_path}"
                )
                # Directly call launch with necessary parameters
                browser = p.chromium.launch(
                    executable_path=nix_chromium_path, headless=True, env=current_env
                )
                ...
                nix_chromium_output = (
                    f"Success. Version: {version}. Loaded page title: '{title}'."
                )
                logger.info("NIX Chromium test successful.")

        except ImportError as e:
            nix_chromium_working = False
            nix_chromium_output = f"Import Error: {e}. Playwright installed?"
            logger.error(f"ImportError during NIX Chromium test: {e}", exc_info=True)
        except Exception as e:
            nix_chromium_working = False
            nix_chromium_output = f"Test Failed: {e}"
            logger.error(f"Exception during NIX Chromium test: {e}", exc_info=True)

    elif not nix_chromium_path:
        nix_chromium_output = "Var PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH not set."
        logger.warning(nix_chromium_output)
        health_data["notes"].append(nix_chromium_output)
    else:
        nix_chromium_output = f"Path '{nix_chromium_path}' set but file doesn't exist."
        logger.warning(nix_chromium_output)
        health_data["notes"].append(nix_chromium_output)

    health_data["checks"]["nix_chromium"] = {
        "status": "success" if nix_chromium_working else "failure",
        "output": nix_chromium_output,
    }
    if not nix_chromium_working:
        health_data["notes"].append("NIX Chromium test failed.")

    # --- 2. System Resources ---
    logger.debug("Diagnostic Check 2: System Resources")
    health_data["checks"]["system_resources"] = get_system_resources(
        logger
    )  # Pass logger
    ...
    playwright_env_vars = {
        "PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS": os.environ.get(
    ...
    health_data["environment_details"]["playwright_env_vars"] = {
        k: (v if v is not None else "not set") for k, v in playwright_env_vars.items()
    }
    chromium_perms = "n/a"
    if nix_chromium_path and os.path.exists(nix_chromium_path):
        try:
            chromium_perms = (
                "executable"
                if os.access(nix_chromium_path, os.X_OK)
                else "not_executable"
            )
        except Exception as e:
            chromium_perms = f"check_failed: {e}"
    elif nix_chromium_path:
        chromium_perms = "path_does_not_exist"
    else:
        chromium_perms = "path_not_set"
    health_data["environment_details"]["playwright_env_vars"][
        "NIX_CHROMIUM_PERMISSIONS"
    ] = chromium_perms
    ...
    try:
        ...
        try:
            ...
            with sync_playwright() as p:
                packages_status["playwright"]["browsers"] = {
                    "chromium": "available" if hasattr(p, "chromium") else "missing"
                }
        except Exception as browser_error:
            packages_status["playwright"]["browsers_check_error"] = str(browser_error)
            logger.warning(
                f"Error checking Playwright browser availability: {browser_error}"
            )
        ...
        health_data["notes"].append("Playwright package not found.")
    try:
        import browser_use

        packages_status["browser_use"] = {
            "status": "installed",
        ...
        try:
            ...
            packages_status["browser_use"]["agent_init_error"] = str(param_error)
            logger.warning(f"Error inspecting browser_use.Agent params: {param_error}")
    except ImportError as import_error:
        packages_status["browser_use"] = {
            "status": "missing",
            "error": str(import_error),
        }
        health_data["notes"].append("browser-use package not found.")
    try:
        import psutil

        packages_status["psutil"] = {
            "status": "installed",
            "version": getattr(psutil, "__version__", "unknown"),
        }
    except ImportError as import_error:
        packages_status["psutil"] = {"status": "missing", "error": str(import_error)}
            ...
            if result.returncode == 0:
                ...
                for lib in libs_to_check:
                    if lib in ldconfig_output:
                        ...
                        system_deps[lib] = "not_found"
                logger.info(
                    f"Found {found_count}/{len(libs_to_check)} potential system dependencies via ldconfig."
                )
            else:
                error_msg = f"'ldconfig -p' failed: {result.stderr}"
                system_deps["error"] = error_msg
                logger.error(error_msg)
                health_data["notes"].append("Failed ldconfig check.")
        except FileNotFoundError:
            error_msg = "'ldconfig' not found."
            system_deps["error"] = error_msg
            logger.warning(error_msg)
            health_data["notes"].append(error_msg)
        except subprocess.TimeoutExpired:
            error_msg = "'ldconfig -p' timed out."
            system_deps["error"] = error_msg
            logger.warning(error_msg)
            health_data["notes"].append(error_msg)
        except Exception as e:
            error_msg = f"Error running 'ldconfig -p': {e}"
            system_deps["error"] = error_msg
            logger.error(error_msg, exc_info=True)
            health_data["notes"].append("Error checking ldconfig.")
    else:
