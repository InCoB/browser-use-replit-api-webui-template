        ...
        async def patched_step(self, step_info: Optional[AgentStepInfo] = None) -> None:
            ...
            try:  # Outer try
                ...
                if (
                    ...
                    logger.info(
                        f"Triggering planner based on interval ({self.state.n_steps} % {self.settings.planner_interval} == 0)"
                    )
...
try:
    ...
    print(
        "NOTE: _run_planner patch IS ACTIVE, will force main LLM for planning step."
    )  # Clarify patch is active
    ...
    async def run_agent():
        ...
        if (
            history
            and hasattr(history, "is_successful")
            and callable(history.is_successful)
        ):
            ...
            if last_result_list and not any(r.error for r in last_result_list):
                if (
                    history.history[-1].model_output
                    and history.history[-1].model_output.is_done()
                ):
                    task_successful = history.history[
                        -1
                    ].model_output.get_done_success()
        ...
        final_agent_report = (
            ...
            if history
            else "Agent did not produce a final result."
        )
...
if AGENT_CLASS_AVAILABLE and original_agent_init and hasattr(Agent, "__init__"):
    try:
        setattr(Agent, "__init__", original_agent_init)
    except Exception as e:
        print(f"\nFailed to restore Agent patch (__init__): {e}")
    else:
        print(f"Restored original Agent method: __init__")
