...
try:
    ...
    from browser_use.agent.views import AgentStepInfo, ActionResult # Added ActionResult
...
if AGENT_CLASS_AVAILABLE:
    ...
    if hasattr(Agent, target_method_name_step):
        ...
        async def patched_step(self, step_info: Optional[AgentStepInfo] = None) -> None:
            ...
            logger = getattr(self, 'logger', logging.getLogger(__name__)) # Use agent's logger or default
            ...
            try: # Outer try for general step setup/interruptions
                ...
                try: # --- Inner try for Action Generation/Execution ---
                    ...
                    if self.register_new_step_callback:
                        ...
                        else: callback_func(state, model_output, self.state.n_steps + 1) # n_steps increments later
                    ...
                    await self._raise_if_stopped_or_paused()
                    self._message_manager.add_model_output(model_output) # Add model output before acting
                ...
                # --- Self-Correction Exception Handling ---
                    ...
                    logger.warning(f"⚠️ Step {self.state.n_steps}: Action generation/execution failed: {type(recoverable_error).__name__}. Attempting self-correction.")
                    ...
                    try:
                         ...
                         logger.error(f"Failed to get browser state after error: {state_err}")
                         # Add simpler error message if state cannot be retrieved
                         self._message_manager._add_message_with_tokens(HumanMessage(content="Failed to retrieve browser state after error."))
                    ...
                    result = [ActionResult(error=f"Attempting correction for: {type(recoverable_error).__name__}", include_in_memory=False)]
                    self.state.last_result = result
            ...
            # --- Outer Exception Handling (Interruptions, etc.) ---
                ...
                self.state.last_result = [ActionResult(error='Agent paused/interrupted', include_in_memory=False)]
                # Do not increment step count here
                raise # Re-raise for run loop to handle pause/stop
            except asyncio.CancelledError:
                logger.info('Agent step cancelled.')
                self.state.last_result = [ActionResult(error='Step cancelled', include_in_memory=False)]
                ...
                logger.error(f"❌ Unexpected error during step {self.state.n_steps}: {e}")
                logger.debug(f"Full traceback for unexpected step error:\n{traceback.format_exc()}")
                # Assume this counts as a definitive failure for this step
                self.state.consecutive_failures += 1
                error_msg = AgentError.format_error(e, include_trace=False) # Use helper if available
                result = [ActionResult(error=f"Unexpected Step Error: {error_msg}", include_in_memory=True)]
                self.state.last_result = result
                ...
                if not isinstance(e if 'e' in locals() else None, (InterruptedError, asyncio.CancelledError)):
                ...
                if state and result: # Ensure state was captured and we have some result (even error/correction result)
                    ...
                    if hasattr(self, '_make_history_item'):
                         ...
                         logger.warning("Could not find _make_history_item method to save history.")
                # --- End Replicated End ---
        ...
        print(f"WARN: Could not find method '{target_method_name_step}' on Agent class to patch. Self-correction patch not applied.")
...
try:
    ...
    def patched_playwright_launch(self, **kwargs):
        ...
        if current_chromium_path and os.path.exists(current_chromium_path):
             ...
             print(f"WARNING: Cannot force executablePath, invalid path: {current_chromium_path}")
...
try:
    ...
    planner_llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash", google_api_key=google_api_key) # Keep defined for Agent init
