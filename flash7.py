...
try:
    ...
    print(f"Failed to import ChatGoogleGenerativeAI/Messages: {e}")
    sys.exit(1)
...
try:
    ...
    from browser_use.agent.views import (
        ...
        AgentBrain,
        AgentOutput,
    )  # Added History classes, AgentError back
    from browser_use.agent.message_manager.views import (
        MessageWrapper,
        ManagedMessage,
    )  # Added ManagedMessage, MessageWrapper
    ...
    from browser_use.agent.memory.service import (
        Memory,
        MemorySettings,
    )  # Needed for init patch
...
if AGENT_CLASS_AVAILABLE:
    ...
    if hasattr(Agent, target_method_name_init):
        original_agent_init = getattr(Agent, target_method_name_init)
        patch_logger.info(f"Stored original Agent method: {target_method_name_init}")
        init_sig = inspect.signature(original_agent_init)

        @functools.wraps(original_agent_init)
        def patched_init(self, *args, **kwargs):
            global original_agent_init
            patch_logger.debug("PATCH __init__: Running original init...")
            original_agent_init(self, *args, **kwargs)
            patch_logger.debug(
                "PATCH __init__: Original init finished. Applying memory fix."
            )
            bound_args = init_sig.bind(self, *args, **kwargs)
            bound_args.apply_defaults()
            enable_memory = bound_args.arguments.get("enable_memory")
            memory_interval = bound_args.arguments.get("memory_interval")
            memory_config = bound_args.arguments.get("memory_config")
            if enable_memory:
                ...
                if "MemorySettings" in globals():
                    ...
                    if not hasattr(self, "_message_manager"):
                        patch_logger.error(
                            "PATCH __init__ Error: self._message_manager missing."
                        )
                        return
                    if not hasattr(self, "llm"):
                        patch_logger.error("PATCH __init__ Error: self.llm missing.")
                        return
                    if "Memory" in globals():
                        ...
                        patch_logger.error(
                            "PATCH __init__ Error: Memory class not found."
                        )
                else:
                    patch_logger.error(
                        "PATCH __init__ Error: MemorySettings class not found."
                    )
            else:
                patch_logger.debug(f"PATCH __init__: Setting self.memory to None.")
                self.memory = None
            patch_logger.info("PATCH __init__: Memory initialization fix applied.")

        setattr(Agent, target_method_name_init, patched_init)
        patch_logger.info(f"Applied patch to Agent method: {target_method_name_init}")
    else:
        patch_logger.warning(
            f"Could not find method '{target_method_name_init}' on Agent class to patch."
        )
        ...
        async def patched_step(self, step_info: Optional[AgentStepInfo] = None) -> None:
            ...
            try:  # Outer try
                ...
                if (
                    hasattr(self, "memory")
                    and self.memory
                    and hasattr(self.memory, "settings")
                    and hasattr(self.memory.settings, "interval")
                    and self.state.n_steps % self.memory.settings.interval == 0
                ):
                    if hasattr(self.memory, "create_procedural_memory") and callable(
                        getattr(self.memory, "create_procedural_memory")
                    ):
                        self.memory.create_procedural_memory(self.state.n_steps)
                await self._raise_if_stopped_or_paused()
                # Corrected method call
                if hasattr(Agent, "_update_action_models_for_page"):
                    await Agent._update_action_models_for_page(self, active_page)
                else:
                    logger.warning(
                        "'_update_action_models_for_page' not found on Agent, skipping."
                    )
                ...
                if (
                    ...
                    logger.info(f"Triggering planner interval")
                    if hasattr(self, "_run_planner") and callable(
                        getattr(self, "_run_planner")
                    ):
                ...
                try:  # Inner try for Action Gen/Exec
                    ...
                    ValueError,
                    ValidationError,
                ) as recoverable_error:  # Catch Parsing errors
                    correction_triggered_this_step = True
                    logger.warning(
                        f"⚠️ Step {self.state.n_steps}: Parsing/LLM error: {type(recoverable_error).__name__}. Correcting."
                    )
                    error_summary = (
                    ...
                    try:
                        ...
                        logger.error(f"Failed get state post-error: {state_err}")
                        self._message_manager._add_message_with_tokens(
                            HumanMessage(content="Failed state retrieval.")
                        )
                    if (
                        self.settings.planner_llm and not run_planner_now
                    ):  # Call planner on error
                        logger.info(
                            f"Triggering planner due to error: {type(recoverable_error).__name__}"
                        )
                        if hasattr(self, "_run_planner") and callable(
                            getattr(self, "_run_planner")
                        ):
                        ...
                        if plan:
                            self._message_manager.add_plan(plan, position=-1)
                    logger.info("⏭️ Requesting corrective action (parsing/LLM error)...")
                    result = [
                        ActionResult(
                            error=f"Correcting: {type(recoverable_error).__name__}",
                    ...
                    logger.warning(
                        f"⚠️ Step {self.state.n_steps}: Action failed ('{failed_action_name}'): {type(action_error).__name__}. Correcting."
                    )
                    error_summary = (
                    ...
                    if (
                        ...
                        or "click_element" in failed_action_name
                        or "element" in str(action_error).lower()
                    ):
                        js_suggestion = " If standard input/click failed, consider using 'execute_javascript_action' with correct JS (e.g., querySelector/getElementById + .value= or .click()) as workaround."
                    ...
                    if (
                        ...
                        logger.info(
                            f"Triggering planner due to error: {type(action_error).__name__}"
                        )
                        if hasattr(self, "_run_planner") and callable(
                            getattr(self, "_run_planner")
                        ):
                        ...
                        if plan:
                            self._message_manager.add_plan(plan, position=-1)
                    logger.info(
                        "⏭️ Requesting corrective action (action execution error)..."
                    )
                    result = [
                        ActionResult(
                            error=f"Correcting: {type(action_error).__name__}",
                            include_in_memory=False,
                        )
                    ]
                    self.state.last_result = result
                ...
                self.state.last_result = [
                    ActionResult(error="Agent paused/interrupted")
                ]
                raise
            except asyncio.CancelledError:
                logger.info("Agent step cancelled.")
                self.state.last_result = [ActionResult(error="Step cancelled")]
                raise InterruptedError("Step cancelled")
            except Exception as e:  # Catch unexpected errors in outer try
                logger.error(
                    f"❌ Unexpected outer error step {self.state.n_steps}: {e}"
                )
...
async def execute_javascript_action(code: str, browser: Browser) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(error=error_str, result_details={"js_code": code})
    return ActionResult(
...
class IframeContentParams(BaseModel):  # Needs BaseModel from pydantic
    ...
    extract_text: bool = Field(
        default=True, description="True for text content, False for inner HTML."
    )


@controller.action(
    "Extracts content from within a specified iframe.", param_model=IframeContentParams
)
async def extract_iframe_content(
    params: IframeContentParams, browser: Browser
) -> ActionResult:
    patch_logger.info(f"Attempting iframe extract: {params.iframe_selector}")
    try:
        page: Page = await browser.get_current_page()
        frame_locator = page.frame_locator(params.iframe_selector)
        try:
            await frame_locator.locator(":root").wait_for(state="visible", timeout=5000)
        except Exception:
            return ActionResult(
                error=f"IFrame '{params.iframe_selector}' not found/visible."
            )
        content = ""
        mode = "text" if params.extract_text else "html"
        if params.extract_text:
            content = await frame_locator.locator("body").inner_text(timeout=5000)
        else:
            content = await frame_locator.locator(":root").inner_html(timeout=5000)
        result_str = f"Successfully extracted {mode} content from iframe '{params.iframe_selector}'."
        patch_logger.info(f"IFrame Extraction success (length: {len(content)}).")
        max_len = 2000
        if len(content) > max_len:
            content = content[:max_len] + f"... (truncated)"
        return ActionResult(
            extracted_content=content,
            result_details={"selector": params.iframe_selector, "mode": mode},
        )
    except Exception as e:
        error_str = f"Error extracting from iframe '{params.iframe_selector}': {type(e).__name__}: {e}"
        patch_logger.error(error_str)
        return ActionResult(error=error_str)
...
try:
    ...
    def format_history_for_chat(
        history: Optional[AgentHistoryList], max_steps: Optional[int] = None
    ) -> str:
        if not history or not history.history:
            return "No history available."
        history_summary_lines = ["Agent Run Summary:"]
        steps_to_include = history.history
        if max_steps is not None and len(steps_to_include) > max_steps:
            steps_to_include = steps_to_include[-max_steps:]
            history_summary_lines.append(f"(Showing last {max_steps} steps)")
        else:
            history_summary_lines.append("(Showing ALL steps)")  # Corrected indentation
        for i, step in enumerate(steps_to_include):
            step_num = (
                step.metadata.step_number if step.metadata else f"Unknown (Index {i})"
            )
            goal = "N/A"
            action_summary = "N/A"
            result_summary = "N/A"
            if step.model_output and step.model_output.current_state:
                goal = getattr(step.model_output.current_state, "next_goal", "N/A")
            if step.model_output and step.model_output.action:
                # Safer way to get action name and params
                try:
                    first_action_dump = step.model_output.action[0].model_dump(
                        exclude_unset=True
                    )
                    ...
                    if (
                        primary_param_val
                        and isinstance(primary_param_val, str)
                        and len(primary_param_val) > 50
                    ):
                        primary_param_val = primary_param_val[:47] + "..."
                    action_summary = (
                        f"{action_name}({primary_param_val})"
                        if primary_param_val
                        else f"{action_name}()"
                    )
                    if len(step.model_output.action) > 1:
                        action_summary += "..."
                except Exception:
                    action_summary = "Error parsing action"  # Handle potential errors getting action details
            if step.result:
                first_result = step.result[0]
                if first_result.error:
                    result_summary = f"Error: {first_result.error[:100]}{'...' if len(first_result.error) > 100 else ''}"
                elif first_result.extracted_content:
                    result_summary = f"Extracted: {first_result.extracted_content[:100]}{'...' if len(first_result.extracted_content) > 100 else ''}"
                elif first_result.is_done:
                    result_summary = f"Done(success={first_result.success})"
                else:
                    result_summary = "Success (No content/error)"
            history_summary_lines.append(
                f"Step {step_num}: Goal='{goal[:100]}...', Action='{action_summary}', Result='{result_summary}'"
            )
    ...
    async def run_agent():
        ...
        history = None
        final_agent_report = "Agent run did not complete successfully."
        steps_taken = 0
        agent_state = None
        try:
            ...
            final_agent_report = (
                ...
                if history
                else "Agent run finished, but no history object returned."
            )
            ...
            if (
                ...
                and hasattr(agent.state, "history")
            ):
                history = agent.state.history
        if history is None:
            history = AgentHistoryList(history=[])  # Ensure history is iterable
        ...
        print(
            f"Agent task finished {'successfully' if task_successful else 'unsuccessfully'}."
        )
        ...
        chat_history.append(
            SystemMessage(
                content="You are the LLM that just controlled a browser agent. The user wants to chat with you about the completed task."
            )
        )
        if "online_editor_vision_task" in locals():
            print("(Context: Including original task prompt)")
            chat_history.append(
                SystemMessage(
                    content=f"Original task:\n---\n{online_editor_vision_task}\n---"
                )
            )
        formatted_history_summary = format_history_for_chat(
            history, max_steps=None
        )  # Include ALL steps
        ...
        if (
            ...
            and hasattr(agent, "_message_manager")
            and hasattr(agent._message_manager, "state")
            and hasattr(agent._message_manager.state, "history")
        ):
            try:
                if "ManagedMessage" in globals():
                    memory_messages = [
                        m.message.content
                        for m in agent._message_manager.state.history.messages
                        if isinstance(m, ManagedMessage)
                        and hasattr(m, "metadata")
                        and m.metadata.message_type == "memory"
                    ]
                    if memory_messages:
                        agent_memory_summary = memory_messages[-1]
                        print("(Context: Including last procedural memory summary)")
                        chat_history.append(
                            AIMessage(
                                content=f"My last internal summary was: {agent_memory_summary}"
                            )
                        )
            except Exception as mem_e:
                print(f"(Debug: Could not extract memory summary - {mem_e})")
        if final_agent_report:
            chat_history.append(
                AIMessage(
                    content=f"Agent's final report/result was: {final_agent_report}"
                )
            )
        # Start chat
        while True:
        ...
        print("\nAgent Run Analysis:")
        steps_taken = (
            agent_state.n_steps - 1 if agent_state else len(history.history)
        )  # Use agent_state if available
        ...
        if final_agent_report is not None:
            print("Final result:", final_agent_report)
        else:
...
if AGENT_CLASS_AVAILABLE:
    ...
    if original_agent_init and hasattr(Agent, target_method_name_init):
        try:
            setattr(Agent, target_method_name_init, original_agent_init)
        except Exception as e:
            print(f"\nFailed to restore Agent patch (__init__): {e}")
        else:
            print(f"Restored original Agent method: {target_method_name_init}")
if original_playwright_launch:
    try:
