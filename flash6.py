        ...
        async def patched_step(self, step_info: Optional[AgentStepInfo] = None) -> None:
            ...
            try: # Outer try
                ...
                # --- Self-Correction Exception Handling ---
                    ...
                    except IndexError: pass # Ignore if already removed
                    ...
                    result = [ActionResult(error=f"Attempting correction for: {type(recoverable_error).__name__}", include_in_memory=False)]; self.state.last_result = result; model_output = None # Ensure model_output is None so history shows error context
            ...
            # --- Outer Exception Handling ---
                ...
                if 'AgentError' in globals() and isinstance(e, Exception): # Check if AgentError is loaded
                ...
                if hasattr(self, 'telemetry') and hasattr(self.telemetry, 'capture'):
                    self.telemetry.capture( AgentStepTelemetryEvent( agent_id=self.state.agent_id, step=self.state.n_steps, actions=actions_taken, consecutive_failures=self.state.consecutive_failures, step_error=step_error ) )
                if state and result:
